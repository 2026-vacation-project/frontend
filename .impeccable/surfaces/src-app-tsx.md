...
한국의 게임 이용자가 현재 파티 모집을 빠르게 찾고 인원과 포지션을 비교해 참가하거나 새 모집을 만든다. 그룹 관리자는 반복적인 파티 구성을 위해 멤버와 역할을 관리한다.
...
게임 파티 라인업 명부. 따뜻한 흰 종이와 짙은 잉크 같은 바탕 위에 모집을 가로 행으로 정렬하고, 참가 인원을 슬롯 레일로 보여준다. 짙은 초록은 행동과 선택에만, 주황은 마감과 주의에만 사용한다. 카드 그리드와 과도한 배지는 사용하지 않는다.
