...
export function RoomFormPage({ edit = false }: { edit?: boolean }) {
  ...
  return <AuthGate><div className="form-page page-container">
    ...
    {loading ? <LoadingRows count={3} /> : <form className="entity-form" onSubmit={submit}>
      ...
      <div className="form-section"><h2>어떤 게임을 함께할까요?</h2><Field label="게임 이름"><input value={form.game_name} onChange={(event) => setForm({ ...form, game_name: event.target.value })} placeholder="예: 오버워치 2, 발로란트" required maxLength={60} /></Field><InlineNotice title="현재 서버 필드 안내">Swagger에는 별도의 방 이름과 추가 조건 필드가 없어 게임 이름과 역할만 저장합니다.</InlineNotice></div>
      ...
      <div className="form-section"><h2>어떤 포지션이 필요할까요?</h2><Field label="필요 포지션" hint="선택 사항이에요. 그룹 역할 이름과 맞추면 찾기 쉬워집니다."><input value={form.target_role ?? ''} onChange={(event) => setForm({ ...form, target_role: event.target.value })} placeholder="예: 힐러, 정글, 감시자" maxLength={30} /></Field></div>
