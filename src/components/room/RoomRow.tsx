    ...
    return (
        <article className="room-row">
            <div className="room-row__activity">
                ...
                <div>
                    ...
                    {example ? (
                        ...
                        <Link className="room-row__title" to={`/rooms/${room.id}?group=${room.group_id}`}>
                            {room.target_role
                                ? `${room.target_role} 포지션을 찾고 있어요`
                                : '같이 즐길 팀원을 찾고 있어요'}
                        </Link>
            ...
            <div className="room-row__need">
                ...
                {room.target_role ? (
                    <RoleBadge name={room.target_role} color="#ef6a42" />
                ) : (
                    <span className="muted">포지션 무관</span>
                )}
            </div>
            ...
            <div className="room-row__capacity">
                ...
                <div className="slot-rail" aria-label={`${room.target_count}자리 중 ${filledSlots}자리 참여`}>
                    {Array.from({ length: Math.min(room.target_count, 8) }, (_, index) => (
                        <span key={index} className={index < filledSlots ? 'is-filled' : ''} />
                    ))}
                </div>
            ...
            <div className="room-row__action">
                ...
                {onJoin ? (
                    ...
                    <span className="text-link text-link--disabled">모집 완료</span>
                ) : (
                    <Link className="text-link" to={example ? '/login' : `/rooms/${room.id}?group=${room.group_id}`}>
