...
export function Avatar({ name, src, size = 'md' }: { name: string; src?: string | null; size?: 'sm' | 'md' | 'lg' }) {
    return src ? (
        <img className={`avatar avatar--${size}`} src={src} alt={`${name} 프로필`} />
    ) : (
        <span className={`avatar avatar--${size}`} aria-label={`${name} 프로필`}>
            {initials(name)}
        </span>
    )
}
...
export function RoleBadge({ name, color }: { name: string; color: string }) {
    return (
        <span className="role-label">
            <span className="role-label__dot" style={{ backgroundColor: color }} />
            {name}
        </span>
    )
}
...
export function LoadingRows({ count = 4 }: { count?: number }) {
    return (
        <div className="loading-rows" aria-label="목록 불러오는 중" aria-busy="true">
