...
> "같이 할 사람을 찾는 과정에서 발생하는 번거로움을 줄이고, 원하는 태그와 조건에 맞는 사람들과 빠르게 팀을 구성한다."
...
게임이나 태그 색상은 Primary와 분리해서 관리한다.

태그 색상은 사용자가 직접 설정할 수 있다.
...
태그 색상은 작은 dot, text label, badge에 사용한다.
...
> 이 사용자가 어떤 활동을 하고 어떤 태그를 갖고 있는지 빠르게 파악하는 화면
...
현재 기기 로그아웃과 모든 기기 로그아웃을 구분한다. 모든 기기 로그아웃은 기존 토큰을 무효화하고, 해당 사용자를 모든 모집방에서 나가게 한다.
...
마지막 참가자가 나가면 빈 모집방을 삭제한다. 방장이 나갔지만 다른 참가자가 남아 있으면 남은 참가자에게 방장을 넘긴다.
...
Swagger에 정의된 실제 태그 API를 사용한다. 호환성을 위해 API 경로는 `/roles`를 유지한다.
...
태그 생성/수정 UI는 Backend Schema에 존재하는 필드만 전송한다.
...
**빠른 탐색 → 모집 상태 파악 → 태그 확인 → 참가 → 그룹 내 지속적인 팀 구성**
